...
describe '#gift_card_selector' do
  ...
  describe 'with 1 max guest' do
    it 'should find no matches' do
      expect(GiftCardSelector.select(input_path_1, 400, 1)).to eq('Not possible')
    end

    it 'should find match with no remainder' do
      expect(GiftCardSelector.select(input_path_1, 2000, 1)).to eq('Earmuffs 2000')
    end

    it 'should find match with remainder' do
      expect(GiftCardSelector.select(input_path_1, 2500, 1)).to eq('Earmuffs 2000')
    end

    it 'should find match with remainder' do
      expect(GiftCardSelector.select(input_path_1, 1600, 1)).to eq('Headphones 1400')
    end

    it 'should find first match' do
      expect(GiftCardSelector.select(input_path_1, 500, 1)).to eq('Candy Bar 500')
    end

    it 'should find last match' do
      expect(GiftCardSelector.select(input_path_1, 6000, 1)).to eq('Bluetooth Stereo 6000')
    end
  end
